...
export default class EventHandler {
    ...
    static njDelegationHandler(element, selector, fn) {
        const handler = event => {
    ...
    static findHandler(events, handler, delegationSelector = null) {
    ...
    static normalizeParams(originalTypeEvent, handler, delegationFn) {
        const hasDelegation = typeof handler === 'string';
        const originalHandler = hasDelegation ? delegationFn : handler;
    ...
    static addHandler(element, originalTypeEvents, handler, delegationFn, oneOff) {
        ...
        for (const originalTypeEvent of originalTypeEvents.split(' ')) {
            const [hasDelegation, originalHandler, typeEvent] = EventHandler.normalizeParams(originalTypeEvent, handler, delegationFn);
            ...
            const uid = EventHandler.getUidEvent(originalHandler, originalTypeEvent.replace(EventHandler.NAMESPACE_REGEX, ''));
    ...
    static removeHandler(element, events, typeEvent, handler, delegationSelector) {
    ...
    static removeNamespacedHandlers(element, events, typeEvent, namespace) {
    ...
    static on(element, events, handler, delegationFn) {
        EventHandler.addHandler(element, events, handler, delegationFn, false);
    }
    static one(element, events, handler, delegationFn) {
        EventHandler.addHandler(element, events, handler, delegationFn, true);
    }
    static off(element, originalTypeEvent, handler, delegationFn) {
        if (typeof originalTypeEvent !== 'string' || typeof element === 'undefined' || element === null) {
            return;
        }
        const [delegation, originalHandler, typeEvent] = EventHandler.normalizeParams(originalTypeEvent, handler, delegationFn);
...
EventHandler.NAMESPACE_REGEX = /[^.]*(?=\..*)\.|.*/;
EventHandler.STRIPNAME_REGEX = /\..*/;
EventHandler.KEYEVENT_REGEX = /^key/;
EventHandler.STRIPUID_REGEX = /::\d+$/;
EventHandler.EVENTREGISTRY = {}; // Events storage
EventHandler.uidEvent = 1;
